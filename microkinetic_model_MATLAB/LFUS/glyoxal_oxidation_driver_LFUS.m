...
path_in = "";
exp_table = readtable(strcat(path_exp,'exp_data.csv'));
...
out_paths = strcat(path_out,'/out_paths.txt');  % Replace with the desired file name
fileID = fopen(out_paths, 'w');
...
extents_table = readtable(strcat(path_in,'generate_network/rxn_extents.csv'));
%stoichiometric coefficients
coeffs_table = readtable(strcat(path_in,'generate_network/rxn_coeffs.csv'));
%rate constants
ks_table = readtable(strcat(path_in,'generate_network/rxn_ks.csv'));
%structures from SMILES
smiles_table = readtable(strcat(path_in,'generate_network/smiles.csv'));
...
beta = [0.7329013762018	1.3958966374997	3.61099563388767];
...
if param_analy==1;
    ...
    %K_eq O2- addition rxn from G_TS of 17.1335259
    param(3,:)=param(3,:).*ks(27)/(exp(-17.1335259/T/R_ig))
...
name1 = "outputs/C1_trend_exp.csv"
name2 = "outputs/C1_trend_model.csv"
%C1_yields_outs = C1_yields_plot(model,exp,C_gly0,tspan,ts,name1,name2);
...
hold off    
save('regress_nOH_and_kgly_wO2n_H2O2_SC_ODE23s_redo')
...
%name to export trends
name = 'outputs/form_acid_trends_LFUS_fit.csv'
...
ylim([.99,1.01])
xlim([0,0.1])
xlabel('Extent of glyoxal consumption','FontSize', f, 'FontName', 'Arial');
...
save('sim_results_5march')
%difference between forward and reverse rate, or rate ratio?
...
%% HFUS OH rate with LFUS model
% concentration and selectivity comparisons for fig 7 (selectivity) and 8 
% (concentrations) for LFUS model
% with 580 kHz OH formation rate
get_LFUS = 0
if get_LFUS == 1
    beta_HFUS_fit = 0.7
    name = 'outputs/form_acid_trends_LFUS_HFUS_val.csv'
    n_OH_fit_LFUS = [2500e-6]/3600/C_gly0*beta_HFUS_fit; %fit value from HFUS
    
    %run simulation
    options=odeset('RelTol',10^-6,'AbsTol',10^-6);
    [t,y]=ode23s(@(t,y) rxn_network(t,y,n_OH_fit_LFUS,0,ks_nd,ks_fit)...
        ,tspan,C0s_nd);
    ...
    CO2s = (y(:,ind_CO2)+y(:,ind_bicarb)+y(:,ind_carbonic))*C_gly0
    glys = (y(:,ind_gly)+y(:,ind_gly_dehyd))*C_gly0
    OHs = n_OH_fit_LFUS*t*C_gly0*1000

    %make conversion plot for C1 yields
    model = [{glys},{glyoxs},{oxs},{forms},{CO2s}];
    expr = [{C_gly0-C_gly_cons},{C_glyox},{C_ox},{C_form}];
    name1 = "outputs/C1_trend_exp_LFUS.csv"
    name2 = "outputs/C1_trend_model_LFUS.csv"
    
    out = rates_out(y,ks_nd,coeffs,extents,ks_fit,ind_fit) 
    rates_species=out{1};
    rates_matrix = out{2};

    %ratio for selectivity
    r_C2s_out = (rates_species(:,ind_glyox) + rates_species(:,ind_glyox_n)+ ...
    rates_species(:,ind_ox) + rates_species(:,ind_ox_n1) + rates_species(:,ind_ox_n2)).*C_gly0.*1000;
    %get forms rates
    r_forms_out = (rates_species(:,ind_form) + rates_species(:,ind_form_n)).*C_gly0.*1000;
    1;
    gly_cons = (C_gly0-glys)*1000;
    S_C2_20_580 = r_C2s_out./r_forms_out

    %% fig 9 output concentrations
    % glyoxylate, O2-, OH, H2O2
    [t*n_OH_fit_LFUS*C_gly0,(C_gly0-glys),y(:,ind_OH)*C_gly0.*1E9,y(:,ind_OOn).*C_gly0.*1000,...
    y(:,ind_glyox_n)*C_gly0,y(:,ind_H2O2)*C_gly0].*1000
end
